...
const VideoStudio: React.FC<VideoStudioProps> = ({ category, onCategoryChange }) => {
  ...
  // History State
  const [history, setHistory] = useState<{ url: string, resource?: any, timestamp: number, name: string }[]>([]);
  ...
  const handleGenerate = async () => {
    ...
    try {
        ...
        // Use the previewUrl directly if it's base64 (not blob), otherwise we need to convert file to base64
  ...
  const handleExtend = async () => {
      ...
      try {
          ...
          setCurrentVideoUrl(result.url);
          setCurrentVideoResource(result.videoResource);
          setExtensionPrompt(''); // Reset prompt
          ...
          // Re-show input if it failed so they can try again or correct prompt
  ...
  const handleSelectHistory = (item: { url: string, resource?: any }) => {
      setCurrentVideoUrl(item.url);
      setCurrentVideoResource(item.resource); // Might be undefined for old history, which disables Extend
      setShowExtendInput(false);
      setError(null);
  };
                 ...
                 <div className="bg-gray-900 rounded-2xl shadow-xl overflow-hidden border border-gray-800 relative flex flex-col min-h-[400px] sm:min-h-[500px] lg:h-[650px]">
                    ...
                    <div className="bg-gray-800/80 backdrop-blur-md px-4 py-3 flex flex-wrap gap-2 justify-between items-center border-b border-white/5 z-20">
                         ...
                         <div className="flex items-center gap-2">
                             {/* Extend Button */}
                             {currentVideoUrl && currentVideoResource && !showExtendInput && (
                                <button
